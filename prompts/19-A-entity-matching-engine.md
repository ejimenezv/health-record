...
## Objective
Implement the Entity Matching Engine for MedRecord AI to handle incremental extraction with intelligent matching and conflict resolution. This prevents duplicate entities and maintains data coherence during real-time streaming.

## Context
**From Architecture (Prompt 06):**
- Entity Matching Engine uses semantic similarity (embeddings) + business rules
- Matching thresholds: >0.85 = strong match, 0.70-0.85 = validate with rules, <0.70 = new entity
- Supports version history with changelog
- Handles symptoms, diagnoses, and prescriptions with type-specific rules

**From Real-Time Pipeline (Prompt 07):**
- Entities are extracted incrementally during streaming
- New mentions may refer to previously extracted entities (e.g., "el dolor de cabeza" → "cefalea")
- Conflicts arise when doctor corrects/clarifies information (e.g., "8/10" → "9/10 severity")
- Version history must be maintained for audit and medical-legal purposes

**BSG Delivery 2 Requirements:**
This maps to Section 3.3 (API Specification) and provides the technical blueprint for entity matching implementation.

## Tasks

### 1. Create Entity Matching Implementation Document
Create `docs/delivery-2/07-entity-matching-engine.md`:
...
2. Threshold-based decision tree is explained (0.85, 0.70 cutoffs)
3. Business rules for each entity type are defined
4. Merge strategies with version control are detailed
5. Changelog mechanism is specified
6. Integration with extraction service is shown
7. Examples of matching scenarios are provided
8. Testing approach is covered
9. Metrics and observability are defined

## Notes
- This is critical for maintaining data coherence in real-time streaming
- Semantic similarity (embeddings) + business rules hybrid approach
- Version history is essential for medical-legal audit trail
- Different thresholds for different confidence levels
- Type-specific business rules (symptom location, medication name, ICD-10 codes)
- Maps to BSG Delivery 2, Section 3.3 (API Specification)
